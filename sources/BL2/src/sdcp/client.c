...
int sdcpcli_keygen(sdcp_keypair* keypair)
{
    ...
    mbedtls_entropy_init(&entropy);
//	printf("\r\n 1");
//	printf("\r\n entropy.source_count 00 = %d ", entropy.source_count);
...
//	printf("\r\n 3\n");
	...
	print_bytes("mbedtls_keypair.d.s", mbedtls_keypair.d.s, sizeof(mbedtls_keypair.d.s));		
...
//	print_bytes("keypair->pk", keypair->pk, sizeof(keypair->pk));
//    printf("\r\n 5");
    ...
    uint8_t* secret, size_t secret_len)
{
	...
	#if CLIEENT_DEBUG
	printf("\r\n pk_len : %d", pk_len);
	print_bytes("pk", pk, pk_len);
	printf("\r\n sk_len : %d", sk_len);
	print_bytes("sk", sk, sk_len);
	printf("\r\n secret_len : %d", secret_len);
	print_bytes("secret 00", secret, secret_len);
	#endif
	...
	//#if 1
	printf("\r\n secret_len : %d", secret_len);
	print_bytes("secret 01", secret, secret_len);
	#endif
exit:
    mbedtls_entropy_free(&entropy);
    mbedtls_ctr_drbg_free(&ctr_drbg);
