                   ...
                   size_t entropy_len )
{
	...
	#if MBED_CTR_DRBG_DEBUG
	print_bytes("mbedtls_ctr_drbg_seed_entropy_len \r\n key00:", key, sizeof(key));		
	#endif
                              ...
                              const unsigned char *additional, size_t add_len )
{
	...
	printf("\r\n ctx->reseed_counter: %d, ctx->reseed_interval: %d, ctx->prediction_resistance: %d\n", ctx->reseed_counter, ctx->reseed_interval, ctx->prediction_resistance);
	#endif
...
//	print_bytes("mbedtls_ctr_drbg_random 00", output, output_len);
	#if 0
	ret = mbedtls_ctr_drbg_random_with_add( ctx, output, output_len, NULL, 0 );
		...
		#endif
		//memset(const_key, 1, 32);
		memcpy(output, const_key, output_len);
