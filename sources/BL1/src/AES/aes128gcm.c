...
//	printf("\r\n dst :"); for (int i = 0; i < 16 ; i++) printf(" %02x", dst[i]);
//	printf("\r\n src :"); for (int i = 0; i < 16 ; i++) printf(" %02x", src[i]);
	*d++ ^= *s++;
	*d++ ^= *s++;
	*d++ ^= *s++;
//	delay_us(10);
...
void init_hash_subkey(const uint8_t* key, uint8_t** H, uint8_t mode) {
	(*H) = (uint8_t*)calloc(16, sizeof(uint8_t));
	aes128e((*H), (*H), key, mode, 16);
//	printf("\r\n init_hash_subkey  key :"); for (int i = 0; i < 32 ; i++) printf(" %02x", key[i]);
//	printf("\r\n init_hash_subkey  H1 :"); for (int i = 0; i < 16 ; i++) printf(" %02x", *H[i]);
}

void init_hash_subkey_d6(const uint8_t* key, uint8_t** H, uint8_t mode) {
	(*H) = (uint8_t*)calloc(16, sizeof(uint8_t));
	dorca3_cipher_decipher(0,1 /*AES*/,key,32,0,(*H),(*H),16,0,1);
//	printf("\r\n init_hash_subkey  key :"); for (int i = 0; i < 32 ; i++) printf(" %02x", key[i]);
//	printf("\r\n init_hash_subkey  H1 :"); for (int i = 0; i < 16 ; i++) printf(" %02x", *H[i]);
	//aes128e((*H), (*H), key, mode, 16);
}
...
/* Prapares data for GCTR procedure. If the input string bit length is zero, no encryption will be applied.
   ...
   */
void prepare_gctr(uint8_t* key, const uint8_t* J, const uint8_t* in, size_t in_len, uint8_t* out, uint8_t mode) {
	uint8_t icb[16];
	...
	//printf("\r\n prepare_gctr in02 : "); for (int i = 0; i < 16 * in_len; i++) printf(" %02x", in[i]);
	gctr(key, icb, in, in_len, out, mode);
	//printf("\r\n dd");
	delay_us(5);
}
/* Encrypts CB_i using AES 128 bit key and then XOR's with X_i - 128 bit string.
   ...
   */
void gctr(uint8_t* key, const uint8_t* icb, const uint8_t* in, size_t in_len, uint8_t* out, uint8_t mode) {
	...
	for (size_t i = 0; i < in_len; i++) {
		...
		aes128e(out_pos, cb, key, mode, 16);
		//printf("\r\n gctr %d, cb :", i); for (int i = 0; i < 16 ; i++) printf(" %02x", cb[i]);
		printf("\r\n gctr %d, out_pos :", i); for (int i = 0; i < 16 ; i++) printf(" %02x", out_pos[i]);
...
void gctr_d6(uint8_t* key, const uint8_t* icb, const uint8_t* in, size_t in_len, uint8_t* out, uint8_t mode) {
	...
	for (size_t i = 0; i < in_len; i++) {
		// Encrypt 16 byte of CB_i using AES 128 bit key and write to out_pos
		//aes128e(out_pos, cb, key, mode, 16);
		dorca3_cipher_decipher(0,1 /*AES*/,key,32,0,out_pos,cb,16,0,1);
		//printf("\r\n gctr %d, cb :", i); for (int i = 0; i < 16 ; i++) printf(" %02x", cb[i]);
		//printf("\r\n gctr %d, out_pos :", i); for (int i = 0; i < 16 ; i++) printf(" %02x", out_pos[i]);
...
void prepare_gctr_d6(uint8_t* key, const uint8_t* J, const uint8_t* in, size_t in_len, uint8_t* out, uint8_t mode) {
	uint8_t icb[16];
	...
	//printf("\r\n prepare_gctr in02 : "); for (int i = 0; i < 16 * in_len; i++) printf(" %02x", in[i]);
	gctr_d6(key, icb, in, in_len, out, mode);
	//printf("\r\n dd");
	delay_us(5);
//	printf("\r\n prepare_gctr in03 : "); for (int i = 0; i < 16 * in_len; i++) printf(" %02x", in[i]);
//	printf("\r\n");
}
...
void aes128gcm_enc(unsigned char* ciphertext, unsigned char* tag, const unsigned char* k,
	...
	init_hash_subkey(k, &H, 0);
	
	printf("\r\n H : "); for (int i = 0; i < 16; i++) printf(" %02x", H[i]);
	
	// Generate pre-counter block J
	init_pre_counter_block(IV, &J);
	
//	printf("\r\n J : "); for (int i = 0; i < 16; i++) printf(" %02x", J[i]);
	
	// ciphertext = GCTR_K(inc_32(J), plaintext)
	prepare_gctr((uint8_t*)k, J, plaintext, block_len, ciphertext, 0);
	
	printf("\r\n prepare_gctr plaintext : "); for (int i = 0; i < 16 * block_len; i++) printf(" %02x", plaintext[i]);
	printf("\r\n prepare_gctr ciphertext : "); for (int i = 0; i < 16 * block_len; i++) printf(" %02x", ciphertext[i]);
	
	// S = GHASH_H(A||0^v||C||0^u||[len(A)]_64||[len(C)]_64)	
	prepare_ghash(H, add_data, add_len, ciphertext, block_len, &S);
	
//	printf("\r\n prepare_ghash H : "); for (int i = 0; i < 16; i++) printf(" %02x", H[i]);
//	printf("\r\n prepare_ghash add_data : "); for (int i = 0; i < 16; i++) printf(" %02x", add_data[i]);
//	printf("\r\n prepare_ghash ciphertext : "); for (int i = 0; i < 16 * block_len; i++) printf(" %02x", ciphertext[i]);
	
	// tag = MSB_t(GCTR_K(J, S)) 
	gctr((uint8_t*)k, J, S, 1, tag, 0);
...
//	printf("\r\n J : "); for (int i = 0; i < 16; i++) printf(" %02x", J[i]);
	...
	// ciphertext = GCTR_K(inc_32(J), plaintext)
	prepare_gctr((uint8_t*)k, J, ciphertext, block_len, plaintext, 0);
...
void aes_gcm_enc(unsigned char* ciphertext, unsigned char* tag, const unsigned char* k,
	const unsigned char* IV, const unsigned char* plaintext,
	const unsigned long block_len, const unsigned char* add_data, const unsigned long add_len) {
...
//	printf("\r\n J : "); for (int i = 0; i < 16; i++) printf(" %02x", J[i]);
	...
	// ciphertext = GCTR_K(inc_32(J), plaintext)
	prepare_gctr_d6((uint8_t*)k, J, plaintext, block_len, ciphertext, 0);
...
void aes_gcm_dec(unsigned char* plaintext, unsigned char* tag, const unsigned char* k,
	const unsigned char* IV, const unsigned char* ciphertext,
	const unsigned long block_len, const unsigned char* add_data, const unsigned long add_len) {
...
//	printf("\r\n J : "); for (int i = 0; i < 16; i++) printf(" %02x", J[i]);
	...
	// ciphertext = GCTR_K(inc_32(J), plaintext)
	prepare_gctr_d6((uint8_t*)k, J, ciphertext, block_len, plaintext, 0);
