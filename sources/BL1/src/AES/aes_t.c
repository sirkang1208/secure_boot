...
//암호 키
  const unsigned char aes_test_key[32] = { 0x21, 0x92, 0xB3, 0x94, 0x25, 0xBB, 0xD0, 0x8B, 0x6E, 0x8E, 0x61, 0xC5, 0xD1, 0xF1, 0xBC, 0x9F, 0x42, 0x8F, 0xC5, 0x69, 0xFB, 0xC6, 0xF7, 0x8C, 0x0B, 0xC4, 0x8F, 0xCC, 0xDB, 0x0F, 0x42, 0xAE };
//const unsigned char aes_test_key[16] = { 0x98, 0xff, 0xf6, 0x7e, 0x64, 0xe4, 0x6b, 0xe5, 0xee, 0x2e, 0x05, 0xcc, 0x9a, 0xf6, 0xd0, 0x12 };
//1회용 초기화 키 Nonce
...
//평문 데이터
 const unsigned char plaintext[32] =  "Test encryption and decryption";
...
void aes128_gcm_main()
{
	...
	//암호화 시작!!
	aes128gcm_enc(ciphertext, tag, aes_test_key, IV, plaintext, block_len, add_data, add_len);
	printf("\r\n pt : ");
	...
	//복호화 시작!!
	aes128gcm_dec(decrypted_plaintext, decrypted_tag, aes_test_key, IV, ciphertext, block_len, add_data, add_len);
	printf("\r\nciphertext 01: ");
	for (int i = 0; i < 16 * block_len; i++) {
		printf("0x%02X ", ciphertext[i]);
	}
